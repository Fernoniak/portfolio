...
const data = [{ title: "HTML", desc: "Posługiwanie się atrybutami globalnymi i eventowymi, znacznikami META i większością elementów HTML", number: "01s" }, { title: "CSS", desc: "Tworzenie animacji, szybkie ustawianie elementów na stronie, duża znajomość właściwości i pseudo-klas CSS", number: "02s" }, { title: "Javascript", desc: "Umiejętność tworzenia skryptów, funkcji, animacji on scroll. Operowanie na tablicach, obiektach i danych", number: "03s" }, { title: "React", desc: "Ukończony kurs z podstaw biblioteki React. Używanie useState oraz useEffect. Tworzenie komponentów i czytelnego kodu", number: "04s" }, { title: "Ogólne", desc: "Styczność z programowaniem od ponad 5-ciu lat. Stale rozwijany sposób myślenia i rozwiązywania problemów", number: "05s" }]

const skillsData = data.map((el, idx) => <SkillsData key={`${idx}`} title={`${el.title}`} desc={`${el.desc}`} number={`${el.number}`} />)
function Skills() {
    return (
